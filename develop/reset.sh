...
waituntil 30 ">>> connect postgres" docker-compose exec db pg_isready
...
docker-compose run --rm order_server python -m migrations
docker-compose run --rm merchant_server python -m migrations
docker-compose run --rm pay_server python -m migrations
docker-compose run --rm commodity_server python -m migrations
